import { render, buildTemplate } from '@pennions/jet';
import { getPropertyNames } from '@pennions/jet/src/functions/templating';
...
function _createReactiveModel(object) {
    var handler = {
...
function _renderTemplate(node, identifier, template, rerender) {
    ...
    if (identifier) {
        ...
        if (isPartial || isWrapped) {
            ...
            node.innerHTML = buildTemplate(template, window._jetViewmodel);
            init(identifier);
        }
        else {
            node.innerHTML = render(template, window._jetViewmodel);
        }
...
export const init = function (elementId, viewmodel, onrendered) {
    ...
    if (viewmodel) {
        window._jetViewmodel = _createReactiveModel(Object.assign(window._jetViewmodel, viewmodel));
    }
    ...
    if (rootElement === null) {
        ...
        childNodes.forEach((child) => {
            if (child.nodeName === '#text' && child.textContent.includes('{{#')) {
                ...
                const innerTemplate = buildTemplate(cleanedTemplate, window._jetViewmodel);
